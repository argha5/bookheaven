...
// Modified query to only show books that are visible (not on rent)
$book_query = "SELECT rb.* FROM rent_books rb
               LEFT JOIN partner_books pb ON rb.rent_book_id = pb.rent_book_id
               WHERE (pb.status IS NULL OR pb.status = 'visible')";

if ($selected_genre !== 'all') {
    $book_query .= " AND rb.genre = ?";
}

$book_query .= " ORDER BY rb.title";
    ...
    if ($books_remaining > 0) {
        ...
        if ($check_result->num_rows == 0) {
            ...
            try {
                ...
                // Update partner book status to 'on rent' and add revenue if it's a partner book
                $update_partner_query = "UPDATE partner_books 
                                        SET status = 'on rent', revenue = COALESCE(revenue, 0) + 40
                                        WHERE rent_book_id = ?";
                $stmt = $conn->prepare($update_partner_query);
                $stmt->bind_param("i", $rent_book_id);
                $stmt->execute();
                ...
                $conn->rollback();
                $_SESSION['error'] = "Error adding book to subscription: " . $e->getMessage();
                header("Location: book_add_to_subscription.php?sub_id=$sub_id&plan_type=$plan_type");
                exit();
            }
        } else {
