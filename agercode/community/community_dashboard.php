...
// Get unread message counts per sender for current user
$unread_counts_sql = "SELECT sender_id, COUNT(*) as unread_count 
                     FROM community_messages 
                     WHERE receiver_id = ? AND status != 'read'
                     GROUP BY sender_id";
$unread_counts_stmt = $conn->prepare($unread_counts_sql);
$unread_counts_stmt->bind_param("i", $user_id);
$unread_counts_stmt->execute();
$unread_counts_result = $unread_counts_stmt->get_result();

// Create an array of sender_id => unread_count
$unread_counts = [];
                        ...
                        <?php while ($member = $members_result->fetch_assoc()): ?>
                            <div class="member-item">
                                ...
                                <button class="message-btn"
                                    ...
                                    <?php if (isset($unread_counts[$member['user_id']]) && $unread_counts[$member['user_id']] > 0): ?>
                                        <span class="unread-count"><?php echo $unread_counts[$member['user_id']]; ?></span>
                                    <?php endif; ?>
                                </button>
                            </div>
                        <?php endwhile; ?>
                                    ...
                                    <div class="post-options">
                                        ...
                                        <div class="options-menu">
                                            <div class="option-item"
                                                onclick="editPost(<?php echo $post['id']; ?>, `<?php echo addslashes($post['content']); ?>`)">
