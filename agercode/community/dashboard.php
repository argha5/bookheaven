...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (isset($_POST['create_community'])) {
    ...
    if ($stmt->execute()) {
      ...
      $stmt->execute();
      $stmt->close();
echo"<script>alert('Community created successfully!');</script>";
    ...
    if ($result->num_rows === 0) {
     ...
     echo "<script>alert('You have joined the community!');</script>";
    ...
    header("Location: dashboard.php");
    exit();
  } elseif (isset($_POST['update_community'])) {
    // Update community
    $community_id = $_POST['community_id'];
    $name = $_POST['community_name'];
    $description = $_POST['community_description'];
    ...
    $stmt = $conn->prepare("SELECT * FROM communities WHERE community_id = ? AND created_by = ?");
    $stmt->bind_param("ii", $community_id, $user_id);
    $stmt->execute();
    $result = $stmt->get_result();
    
    if ($result->num_rows === 0) {
      $_SESSION['error_message'] = "You don't have permission to edit this community.";
      header("Location: dashboard.php");
      exit();
    }
    ...
    if (isset($_FILES['cover_image']) && $_FILES['cover_image']['error'] === UPLOAD_ERR_OK) {
      ...
      // Check if image file is a actual image
      $check = getimagesize($_FILES['cover_image']['tmp_name']);
      if ($check !== false && move_uploaded_file($_FILES['cover_image']['tmp_name'], $target_file)) {
        $cover_image_url = '/BookHeaven2.0/assets/community_images/' . $file_name;
      }
    }
    ...
    if ($cover_image_url) {
      $stmt = $conn->prepare("UPDATE communities SET name = ?, description = ?, privacy = ?, cover_image_url = ? WHERE community_id = ?");
      $stmt->bind_param("ssssi", $name, $description, $privacy, $cover_image_url, $community_id);
    } else {
      $stmt = $conn->prepare("UPDATE communities SET name = ?, description = ?, privacy = ? WHERE community_id = ?");
      $stmt->bind_param("sssi", $name, $description, $privacy, $community_id);
    }
    
    if ($stmt->execute()) {
      echo "<script>alert('Community updated successfully!');</script>";
    } else {
        echo "<script>alert('Error updating community: " . $conn->error . "');</script>";
    }
...
// Get user's communities with creator info
$user_communities = [];
$stmt = $conn->prepare("
...
<body>
                ...
                <tr>
                  ...
                  <td>
                    <button class="view-btn"
                      onclick="window.location.href='community_dashboard.php?id=<?= $community['community_id']; ?>'">Go to Feed</button>
            
                    <?php if ($community['created_by'] == $user_id): ?>
                      <!-- <button class="members-btn"
                      onclick="window.location.href='community_members.php?id=<?= $community['community_id']; ?>'">Members</button> -->
  ...
  <div class="modal" id="editModal">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h2 class="modal-title">Edit Community</h2>
      </div>
      <form id="editCommunityForm" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <div>
            ...
            <label for="editPublic">Public (Anyone can join)</label>
          </div>
          <div>
            <input type="radio" id="editPrivate" name="privacy" value="private">
            <label for="editPrivate">Private (Requires approval to join)</label>
          </div>
        </div>
  ...
  <script>
    ...
    function openEditModal(communityId, name, description, privacy) {
    ...
    // Form validation for edit community
