...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['bookCover']) && $_FILES['bookCover']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($file_type, $allowed_types) && $file_size <= 5000000) {
            if (move_uploaded_file($_FILES['bookCover']['tmp_name'], $target_path)) {
                $cover_path = 'assets/rent_book_covers/' . $file_name;
                $has_cover = true;
            }
        }
    }
    ...
    if (empty($title) || empty($writer) || empty($genre) || empty($description) || !$has_cover) {
        $_SESSION['error_message'] = "All fields are required, including a valid book cover image (JPEG, PNG, GIF up to 5MB).";
        header("Location: partner_add_books.php");
        exit();
    }
    ...
    try {
        ...
        if (!$stmt->execute()) {
            throw new Exception("Error inserting into rent_books table");
        }

        $rent_book_id = $conn->insert_id;

        // Insert into partner_books table with status 'pending'
        $insert_partner_book_query = "INSERT INTO partner_books 
                                    (partner_id, rent_book_id, added_at, status, revenue) 
        ...
        if (!$stmt->execute()) {
            throw new Exception("Error inserting into partner_books table");
        }
        ...
        if ($has_cover && file_exists($upload_dir . $file_name)) {
            unlink($upload_dir . $file_name);
        }
        
        $_SESSION['error_message'] = "Error submitting book: " . $e->getMessage();
        header("Location: partner_add_books.php");
        exit();
    }
}
    ...
    <div class="main-container">
        <aside>
            ...
            <nav>
                <ul>
                    ...
                    <li><a href="partner_add_books.php" class="active"><i class="fas fa-book"></i> Add Book</a></li>
                </ul>
            </nav>
        </aside>
        ...
        <main>
            ...
            <div class="card">
                ...
                <form id="bookForm" method="POST" enctype="multipart/form-data" action="partner_add_books.php">
